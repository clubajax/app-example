        ...
        requirejs(["chart/chart", "chart/SeriePainters/SeriePainterFactory", "plugins/resourceManager/ResourceManager", "plugins/interaction/Broker/Broker", "common/userInteractionTypes", "common/utilities", "plugins/interaction/crosshair/crosshair", "plugins/interaction/selection/selection", "plugins/interaction/graphResizing/graphResizing", "plugins/interaction/horizontalZoom/horizontalZoom", "plugins/interaction/verticalZoom/verticalZoom", "plugins/superScroller/HorizontalScrollBar", "plugins/interaction/drawingObjectCreation/drawingObjectCreator", "plugins/interaction/datatip/Datatip", "common/ChartTypes"], function(Chart, seriePainterFactory, resourceMananger, Broker, userInteractionTypes, utilities, Crosshair, Selection, GraphResizing, HorizontalZoom, VerticalZoom, HorizontalScrollBar, DrawingObjectCreator, Datatip, chartTypes) {
            ...
            broker.pluginsStack.push(new HorizontalZoom({
                onRangeChangeCallback: function(range) {
                    //window.superScroller.activeRange(range);
                }
            }));
           ...
           // broker.pluginsStack.push(new Selection());
            ...
            //broker.pluginsStack.push(new GraphResizing({
            //    chartOverlayTemplate: '<div style="background-color:white; opacity:0.24; filter:alpha(opacity=24); border: #C5C5C5 2px solid; z-index:29999"></div>',
            //    graphOverlayTemplate: '<div style="border: yellow 2px dotted; z-index:30000"></div>',
            //    dividerTemplate: '<div style="border-color: #C5C5C5;border-width: 1px 0;background-repeat: repeat-x;background-color: #E3E3E3;cursor: n-resize; position:absolute; height:5px; left:0px; z-index:100000"></div><div style="overflow: hidden;background-color: transparent;background-repeat: no-repeat;font-size: 0;line-height: 0;text-indent: -3333px;text-align: center;vertical-align: middle;background-image: url("http://cdn.kendostatic.com/2011.3.1129/styles/Default/sprite.png");color: #2E2E2E;cursor: n-resize;"></div></div>',
            //    onGraphsResizing: function(newPercentages) {
            //        if (typeof console !== "undefined") {
            //            console.log("resizing: " + JSON.stringify(newPercentages));
            //        }
            //        return true;
            //    },
            //    onGraphsResized: function(newPercentages) {
            //        if (typeof console !== "undefined") {
            //            console.log("resized: " + JSON.stringify(newPercentages));
            //        }
            //        return true;
            //    }
            //}));

//            broker.pluginsStack.push(new DrawingObjectCreator({
...
//                    }
            ...
            timeStamp = graph.axes(0).series(0).data()[0].timeStamp;
            graph.dataInspect(timeStamp);

            var prevViewPortLeftMargin = 0,
            ...
            //var limits = chart.xAxis().getActualLimits(), newLimits = { minValueIndex: limits.minValueIndex, maxValueIndex: limits.maxValueIndex - 10 };
            //chart.xAxis().limits(newLimits);

            var ser = graphSettings[0].axes[0].series[0], axis = chart.graphs(0).axes(0);

            //axis.series.splice(0, 1, ser);
            //axis.series([]);
            axis.series([ser]);

            //graph.hitTest(10, 10);

            //chart.triggerGesture('move', { button: 0, pointers: [{ x: 10, y: 10, id: 0 }] });
