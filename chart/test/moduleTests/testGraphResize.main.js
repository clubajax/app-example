...
require([
    ...
    function (domReady, Chart, seriePainterFactory, resourceMananger, Broker, userInteractionTypes, utilities, GraphResizing) {

        domReady(function () {

            var $container = $('#parent'),
                broker = new Broker();
            //object.settings.template = object.settings.template || '<div style="position:absolute"> </div>';
            //broker.pluginsStack.push(new GraphResizing({ template: '<div style="position:absolute; background-color:red; height:5px; left:0px; z-index:100000" > </div>' }));
            broker.pluginsStack.push(new GraphResizing(
                {
                    //chartOverlayTemplate: '<div style="background-color:white; opacity:0.4; filter:alpha(opacity=40); border: #C5C5C5 2px solid; z-index:29999"></div>',
            ...
            //$('canvas').remove();
            //$('div').css('border', '3px red solid');

            var lastGraph = chart.graphs(2), perc = lastGraph.realEstatePercentage(), direction = 1, newGraphPercentages = [0.33, 0.33, 0.33], inc = 0.01, i = 0, n = 1000,
                lastPerc;
            chart.distributeGraphs(newGraphPercentages);
            var exec = function () {
                if (i++ < n) {
                    lastPerc = lastGraph.realEstatePercentage();
                    if (lastPerc > 0.90 || lastPerc < 0.10) {
                        direction *= -1;
                    }
                    newGraphPercentages[0] -= direction * inc / 2;
                    newGraphPercentages[1] -= direction * inc / 2;
                    newGraphPercentages[2] += direction * inc;
                    chart.distributeGraphs(newGraphPercentages);
                    setTimeout(exec, 0);
                }
            };
