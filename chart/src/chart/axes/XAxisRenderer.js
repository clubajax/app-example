define([
    ...
    'common/Rect',
    'localLib/logger'
], function (dcl, utilities, CanvasPrinter, rect, logger) {

    var
        log = logger('XA', 0, 'XAxisRenderer'),
        usingSpecial = false,
        labelAction = {
    ...
    return dcl(null, {
        ...
        constructor: function(parent, settings){
            ...
            if(!this.themeHandle){
                this.themeHandle = this.settings.theme.on('change', 'render', this);
            }
        ...
        _categorizeLabel: function (prevBarTime, curBarTime, labelDetail) {
            ...
            if (prevBarTime.getTime() === curBarTime.getTime()) {
                log('set xAxis label to Date:', prevBarTime);
        ...
        _preRender: function () {
                    ...
                    for (j = tempLabels.length - 1; j >= 0; j--) {
                        ...
                        if (ilabelAction === labelAction.add) {
                            ...
                            timeAxisLabels.push({
                                ...
                                time: tmp.time,
                                typeLabel:labelLabels[tmp.type],
                                subTypeLabel:labelLabels[tmp.subtype]
                            });
            ...
            window.xAxisLabels = timeAxisLabels;

            if(usingSpecial){
                log('timeAxisLabels', timeAxisLabels);
                log('data:', data.slice(data.length - 10, data.length));
            }
