define([
    ...
    var
        ...
        mouseHandled = false,
        defaultOptions = {
            totalRange: {minValue:0, maxValue:0},
            activeRange: {minValue:0, maxValue:0},
            step: 1,
            pageStep: 5,
            minimumRange: 1,
            cancel: ':input,option'
    ...
    SuperScroller.prototype = {
        ...
        moveRightHandle: function(px){
            var
                amt,
                tr = this.totalRange().maxValue,
                al = this.activeRange().minValue,
                ar = this.activeRange().maxValue,
            ...
            amt = Math.min(amt, this.totalRange().maxValue);
            this.activeRange({minValue:al, maxValue:amt});
        },

        moveLeftHandle: function(px){
            var
                amt,
                tl = this.totalRange().minValue,
                al = this.activeRange().minValue,
                ar = this.activeRange().maxValue,
                unit = this.pixelsToUnits(px);
            ...
            amt = Math.max(amt, this.totalRange().minValue);
            this.activeRange({minValue:amt, maxValue:ar});
        },
        ...
        moveRange: function(px){
            ...
            if(units + al < tl){
                this.activeRange({minValue:tl, maxValue:tl + r});
            }else if(units + ar > tr){
                this.activeRange({minValue:tr - r, maxValue:tr});
            }else{
                this.activeRange({minValue:al + units, maxValue:ar + units});
            }
        ...
        jumpScroll: function(px){
            ...
            var move = function(){
                ...
                this.activeRange({minValue:al + amt, maxValue:ar + amt});
                this.positionDom();
            }.bind(this);
        ...
        pixelsToUnits: function(px){
            var
                range = this.totalRange().maxValue - this.totalRange().minValue,
